...
const SupportingDoc = ({ files, onFilesChange }: SupportingDocProps) => {
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (invalidFiles.length > 0) {
            toast.error('无效的文件类型', {
                description: '请仅上传 PDF、TXT、PPTX 或 DOCX 文件',
            });
            return;
        }

        if (hasPdf && droppedFiles.some(file => file.type === 'application/pdf')) {
            toast.error('不支持多个PDF文件', {
                description: '请仅选择一个PDF文件',
            });
            return;
        }
    ...
    return (
        <div className="w-full">
            <h2 className="text-[#444] font-instrument_sans pt-4 text-lg mb-4">支持文档</h2>
            <div
